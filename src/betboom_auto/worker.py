...
def make_bet(match_id: int, winner: int) -> None:
    ### winner это номер команды(1 или 2) 
    ### match_id это id матча в бетбуме
    logging.info(f"match_id - {match_id}, winner - {winner}")

    # создание зявки на ставку
    bet_application = requests.post('https://api-bifrost.oddin.gg/main/bifrost/query', headers=headers, json=get_application_json(match_id, winner))
    logging.info(f"application status - {bet_application.status_code}")

    # подтверждение ставки
    bet_confirmation = requests.post('https://api-bifrost.oddin.gg/main/bifrost/query', headers=headers, json=get_confirmation_json(match_id, winner))
